...
void LifetimeReporterBase::initializeFiltering(CFG* Cfg, IsCleaningBlockTy ICB)
{
    this->Cfg = Cfg;
...
bool markPathReachable(const CFGBlock* From, const llvm::DenseMap<const CFGBlock*, const CFGBlock*>& ParentMap,
...
template <typename Pred>
bool hasPathThroughNodePred(const CFG& Cfg, const CFGBlock* Source, const CFGBlock* Dest, const Pred& P)
{
...
bool LifetimeReporterBase::shouldBeFiltered(const CFGBlock* Source, const Variable* V) const
{
    ...
    if (hasPathThroughNodePred(
            *Cfg, Source, Current, [V, this](const CFGBlock* B) { return IsCleaningBlock(*B, V); })) {
        return true;
    }
...
class LifetimeContext {
    ...
    struct BlockContext {
        ...
        std::optional<PSetsMap> FalseBranchExitPMap;
    };
    ...
    BlockContext& getBlockContext(const CFGBlock* B) { return BlockContexts[B->getBlockID()]; }
    ...
    void dumpBlock(const CFGBlock& B) const
    {
        ...
        if (Loc.isValid()) {
            ...
            llvm::errs() << "Block at " << SourceMgr.getBufferName(Loc) << ":" << SourceMgr.getSpellingLineNumber(Loc)
                         << "\n";
        }
...
public:
    LifetimeContext(ASTContext& ASTCtxt, LifetimeReporterBase& Reporter, const FunctionDecl* FuncDecl,
        IsConvertibleTy IsConvertible)
        : ASTCtxt(ASTCtxt)
        , FuncDecl(FuncDecl)
        , AC(nullptr, FuncDecl)
        ...
        , IsConvertible(IsConvertible)
    {
        ...
        if (Reporter.shouldFilterWarnings()) {
            ...
            auto IsCleaningBlock = [this](const CFGBlock& B, const Variable* V) {
                ...
                if (!PSetOfVarBefore->second.containsNull() && !PSetOfVarBefore->second.containsInvalid()) {
                    return false;
                }
                if (PSetOfVarAfter->second.containsNull() || PSetOfVarAfter->second.containsInvalid()) {
                    return false;
                }
                return true;
            };
...
void LifetimeContext::computeEntryPSets(const CFGBlock& B)
{
    ...
    if (B.succ_size() == 2 && isNoopBlock(B) && llvm::all_of(B.preds(), [this](const CFGBlock::AdjacentBlock& Pred) {
    ...
    for (const auto& PredBlock : B.preds()) {
        ...
        if (PropagateFalseSet) {
            ...
            auto& PredPMap
                = (PredBlock->succ_size() == 2 && *PredBlock->succ_rbegin() == &B && PredBC.FalseBranchExitPMap)
...
static bool shouldSuppressLifetime(const FunctionDecl* Func)
{
    ...
    return llvm::any_of(Attr->diagnosticIdentifiers(), [](StringRef Identifier) { return Identifier == "lifetime"; });
}
